...
you don't have to configure any path; everything is already set up inside the docker container which means if you just type `telemac2d.py` 
the command should already be understood. 


Now in order to run an example we are going to extract the case we are interested in the /opt folder
